		...
		>
			{items.map((item, index) => (
				<button
					...
					className={`relative bg-transparent outline-none w-[5em] h-[5em] [perspective:24em] [transform-style:preserve-3d] [-webkit-tap-highlight-color:transparent] group ${
						item.customClass || ""
					}`}
				>
					<span
					...
					>
						<span
							className="w-[2em] h-[2em] sm:w-[2.5em] sm:h-[2.5em] flex items-center justify-center"
							aria-hidden="true"
						>
							{React.cloneElement(item.icon, {
								className: "text-white text-[1.5em] sm:text-[2em]",
							})}
						</span>
					</span>
